...
def main(fdir):
    ...
    vars = ['C', 'DO mg/L', 'SAL-PSU', 'pH']
    for f in files:
        df = pd.read_csv(f, skiprows=14)
        
        # remove rows of data where SAL is < 30
        df = df[df['SAL-PSU'] >= 30]
        df = df.reset_index(drop=True)

        # find max depth minus 0.05m, find the median index at that depth and assign down/up profiles
        deep = np.nanmax(df['DEP m'])
        deep_range = deep - .05

        deep_idxs = np.where(np.logical_and(df['DEP m'] >= deep_range, df['DEP m'] <= deep))[0]
        ...
        for v in vars:
            ...
            for c, color in cast.items():
                depth = df.loc[df['profile'] == c, 'DEP m'].values
            ...
            ttl = f'{fname.split('.csv')[0]}:  {df.Date[0]} {df.Time[0]}'
            ax.set_title(ttl)
...
if __name__ == '__main__':
    filedir = '/Users/garzio/Documents/rucool/Saba/CostaRica/ProDSS'
    main(filedir)
